...
local LOCAL_USER_MAP = {
  pins = {
    ["1234"] = { name = "User 1", index = 1 },
  },
  rfids = {
    ["ABCD1234"] = { name = "User 2", index = 2 },
  },
}
...
local function get_user_map(device)
  local map = device:get_field("securitySystem_user_map")
  if map == nil then
    map = update_user_map_from_prefs(device, LOCAL_USER_MAP)
...
local function sync_lock_codes_from_user_map(device, map)
  ...
  local lock_pins = utils.deep_copy(get_lock_code_pins(device))

  for slot, pin in pairs(lock_pins or {}) do
    if map.pins[pin] == nil then
      lock_pins[slot] = nil
      lock_codes[slot] = nil
  ...
  for pin, entry in pairs(map.pins or {}) do
    if entry.index ~= nil then
      local slot = tostring(entry.index)
      lock_codes[slot] = entry.name or lock_codes[slot] or ("Code " .. slot)
      lock_pins[slot] = pin
    end
  end
...
local function handle_arm_away(driver, device, command)
  ...
  emit_arm_activity(device, "armedAway", "App")
...
local function handle_arm_stay(driver, device, command)
  ...
  emit_arm_activity(device, "armedStay", "App")
...
local function device_init(driver, device)
  ...
  local map = update_user_map_from_prefs(device, base_map)
  device:set_field("securitySystem_user_map", map, { persist = true })
